...
public abstract class FastBufferedImage extends Blittable implements Paintable {
        ...
        if ((db instanceof DataBufferInt) || !forceIntBuffer) {
            ...
            for (int i = 0; i < len; i++) {
                ...
                if (img.getType() == BufferedImage.TYPE_4BYTE_ABGR) {
                    buffer[i] = merge(bs[i * 4], bs[i * 4 + 3], bs[i * 4 + 2], bs[i * 4 + 1]);
                } else {
                    buffer[i] = merge(bs[i * 4 + 3], bs[i * 4 + 2], bs[i * 4 + 1], bs[i * 4]);
    ...
    // copied from bitops to remove dependencies to gui
    private static int merge(int i, int j, int k, int l) {
        return ((cleanByte(i) << 24) | (cleanByte(j) << 16) | (cleanByte(k) << 8) | cleanByte(l));
    }

    private static int cleanByte(int b) {
        return b >= 0 ? b : b + 256;
    }
}
